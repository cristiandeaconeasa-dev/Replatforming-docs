...
| 3 | [03 — Replatforming Deep Dive](03-replatforming-deep-dive.md) | Why we're replatforming, the DTOflow platform, the phases, Shadow Mode, the live epic backlog and the critical blocker. | …to lead the program |
...
| 15 | [15 — Overall Status v2](15-overall-status-v2.md) | Complete status using the Workstream → Capability hierarchy. Explains the Phase → Workstream → Capability → Epic organisation, includes both summary tables and the full two-level epic mapping (58 epics across 5 workstreams × 5 capabilities). Designed for regular status presentations. | …to present program status to CTO / PMO / stakeholders |
| 15a | [15 — Overall Status (original)](15-overall-status.md) | The previous version using the old 10 "Dimensions" organisation — kept for reference. | …to compare with the old format |
| 16 | [16 — Docs GitHub Strategy](16-docs-github-strategy.md) | Strategy to move all Pricer documentation from Confluence to GitHub — docs-as-code with MkDocs Material, per-repo `docs/` convention, centralized hub via `evo-docs`, migration plan, and repo prioritization. | …to plan and execute the docs migration |
...
| 18 | [18 — Docs GitHub Implementation](18-docs-github-implementation.md) | Concrete implementation guide for the docs-as-code platform: GCP infrastructure setup (Cloud Run + IAP + WIF), complete evo-docs hub repo setup (all files copy-paste ready), creating new repos with docs, adopting existing repos, adding/removing docs, CI/CD pipeline details, maintenance procedures, and troubleshooting. | …to implement the docs platform |
| 19 | [19 — Dimension Frameworks](19-dimension-frameworks.md) | Analysis of why the current 10 "Dimensions" don't work (mixed abstraction levels, no consistent naming) and two proposed alternative frameworks: **Framework A** (Delivery Workstreams — for CTO/PMO) and **Framework B** (System Capabilities — for engineers/architects). Includes full epic-to-category mappings for both. | …to decide how to reorganize the Executive Summary |
